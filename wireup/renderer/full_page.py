...
<body>
  ...
  <script>
    ...
    const nodeParentById = Object.fromEntries(
      graphData.nodes.map((node) => [node.id, node.parent || null])
    );
    ...
    graphData.nodes.forEach((node) => {
      if (node.parent && !groupMap.has(node.parent)) {
        groupMap.set(node.parent, { id: node.parent, label: node.group, kind: "group", module: node.group });
      }
    });
    const elements = [
      ...[...groupMap.values()].map((group) => ({ data: group })),
    ...
    function applyModuleOrganization() {
      cy.batch(() => {
        cy.nodes().forEach((node) => {
          ...
          const originalParent = nodeParentById[node.id()] || null;
...
def render_graph_page(container: BaseContainer, *, title: str, options: GraphOptions) -> str:
