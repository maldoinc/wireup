...
def test_to_graph_data_returns_expected_groups_nodes_and_edges() -> None:
    ...
    group_ids = {node.parent for node in graph.nodes if node.parent}
    assert group_ids >= {
        "group_Configuration",
        "group_FastAPI",
        "group_factories",
    ...
    assert (
        nodes_by_id["pet_store_demo_app_services_pets_PetCatalogService"].module == "pet_store_demo_app.services.pets"
    )
