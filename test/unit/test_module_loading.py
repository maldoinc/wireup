...
class ModuleLoadingTest(unittest.TestCase):
    def test_warmup_loads_all_in_module_with_annotations(self):
        container = wireup.create_sync_container(service_modules=[with_annotations], config={"env_name": "dev"})
    ...
    def test_loads_module_is_file(self):
        ...
        container = wireup.create_sync_container(config={"env_name": "dev"}, service_modules=[services])
