...
Instead of manually retrieving services via `container.get` or configuration via `container.config`, you can inject them
directly into function parameters using the container as a decorator. Works with both sync and async containers.
...
Use the `inject_from_container` decorator to automatically inject dependencies into function parameters. The container
...
The decorator works seamlessly with `async` functions, however the container must be created using
